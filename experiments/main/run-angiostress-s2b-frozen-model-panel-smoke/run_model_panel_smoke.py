#!/usr/bin/env python3
"""Frozen two-model panel smoke for AngioStress synthetic and DIAS surfaces."""
...
RUN_ID = "run-angiostress-s2b-frozen-model-panel-smoke"
BASELINE_METRICS = {
...
MODEL_PANEL = [
    {
        ...
        "checkpoint": "tmp/angiostress-s1-weights/sam_vit_b_01ec64.pth",
    ...
    },
    {
        ...
        "checkpoint": "tmp/angiostress-s2b-weights/medsam/medsam_vit_b.cpu.pth",
...
def ordering_comparison(summary_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if len(synthetic_order) != 2 or len(dias_order) != 2:
        agreement = None
    else:
        agreement = 1.0 if synthetic_order == dias_order else -1.0
    return {
        ...
        "two_model_order_agreement": agreement,
...
def evaluate_all(args: argparse.Namespace, device: str) -> dict[str, Any]:
    ...
    metrics: dict[str, float] = {
        **BASELINE_METRICS,
        "s2b_model_count": float(len(MODEL_PANEL)),
        "s2b_synthetic_item_count_per_model": float(len(synthetic_inputs)),
        "s2b_dias_frame_count_per_model": float(len(dias_inputs)),
        "s2b_total_prediction_count": float(len(synthetic_rows) + len(dias_rows)),
        "s2b_ranked_model_count": float(len(summary_rows)),
        "s2b_finite_metric_check": 1.0,
    }
    for row in summary_rows:
        prefix = f"s2b_{row['model_id']}"
        for key, value in row.items():
            if key == "model_id":
                continue
            metrics[f"{prefix}_{key}"] = float(value)
    if order_compare["two_model_order_agreement"] is not None:
        metrics["s2b_two_model_order_agreement_synthetic_mean_vs_dias"] = float(order_compare["two_model_order_agreement"])
    metric_values = [v for v in metrics.values() if isinstance(v, (float, int))]
    if not all(np.isfinite(metric_values)):
        metrics["s2b_finite_metric_check"] = 0.0
        raise ValueError("nonfinite metric detected")
    write_json(args.out / "metrics_summary.json", metrics)

    manifest = {
        "run_id": RUN_ID,
        "parent_run_id": "run-angiostress-s3-dias-frozen-model-analog-smoke",
        "device": device,
        ...
        "claim_boundary": (
            "Frozen-model panel smoke only. This verifies two frozen public models can be evaluated on shared "
...
- Model panel: SAM ViT-B and MedSAM ViT-B, both frozen.

## Method

Both models are evaluated through `segment_anything` `SamPredictor` with identical label-derived bounding-box prompts. The synthetic surface uses the deterministic TopCoW projection and stressor cells; the real surface uses DIAS `s40` frames and deterministic frame-subset analogs.

## Claim Boundary

This is a frozen-model panel smoke. It creates comparable two-model ordering rows, but it does not estimate construct validity, confidence intervals, or clinical accuracy.
"""
...
- outputs_exist: true
- finite_metric_check: `{metrics["s2b_finite_metric_check"]}`
- model_count: `{metrics["s2b_model_count"]}`
- synthetic_item_count_per_model: `{metrics["s2b_synthetic_item_count_per_model"]}`
- dias_frame_count_per_model: `{metrics["s2b_dias_frame_count_per_model"]}`
- total_prediction_count: `{metrics["s2b_total_prediction_count"]}`
- ranked_model_count: `{metrics["s2b_ranked_model_count"]}`
